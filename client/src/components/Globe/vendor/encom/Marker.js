...
var Marker = function(lat, lon, text, altitude, previous, scene, _opts){
    ...
    this.labelSprite.scale.set(labelCanvas.width, labelCanvas.height, 1);
    ...
    .onUpdate(function(target){
        _this.marker.scale.set(target.x, target.y, 1);
    })
  ...
  if(this.previous){
      ...
      var materialSpline,
        ...
        _this.geometrySpline = new LineGeometry();
        ...
        _this.geometrySplineDotted = new LineGeometry();
        ...
        update = function(){
            ...
            for(var x = 0; x< _this.geometrySpline.vertices.length; x++){
                ...
                const currentVert = _this.geometrySpline.vertices[x];
                const currentPoint = utils.mapPoint(nextSpot.lat, nextSpot.lon);
                ...
                if (_this.dashedLine) _this.dashedLine.computeLineDistances();
            }
        ...
        _this.geometrySpline.sync();
        _this.geometrySplineDotted.sync();
        const topLine = new Line(_this.geometrySpline.buffer, materialSpline);
        const dashedLine = new Line(_this.geometrySplineDotted.buffer, materialSplineDotted);
        dashedLine.computeLineDistances();
        dashedLine.frustumCulled = false;
        topLine.frustumCulled = false;
        _this.topLine = topLine;
        _this.dashedLine = dashedLine;
...
Marker.prototype.remove = function(){
    if (this.topLine) this.scene.remove(this.topLine);
    if (this.dashedLine) this.scene.remove(this.dashedLine);
